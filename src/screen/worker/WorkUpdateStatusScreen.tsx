...
const WorkUpdateStatusScreen = () => {
  ...
  const handleUpdate = async () => {
    ...
    // If the project is completed, store it in AsyncStorage
    if (updatedCompletion === 100) {
        const completedProject = { 
            ...project, 
            completion_percentage: 100,  // Ensure this is explicitly updated
            status: "Completed" 
        };

        try {
            // Retrieve existing stored projects
            const storedProjects = await AsyncStorage.getItem("completedProjects");
            let projects = storedProjects ? JSON.parse(storedProjects) : [];

            // Remove any existing entry for the same project (to avoid duplicates)
            projects = projects.filter((p: Project) => p.project_Id !== project.project_Id);

            // Append the updated completed project
            projects.push(completedProject);

            // Save the updated list back to AsyncStorage
            await AsyncStorage.setItem("completedProjects", JSON.stringify(projects));
    ...
    // Navigate back for non-completed projects
    navigation.goBack();
};
